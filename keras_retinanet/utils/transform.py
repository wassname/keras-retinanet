from  keras.preprocessing import transform_matrix_offset_center, apply_transform
import numpy as np
...
def random_rotation(max):
    ...
    """
    return rotation(np.random.uniform(-max, max))
...
def random_translation(max):
    """ Construct a random 2D translation between -max and max.
    # Arguments
        max: the maximum absolute translation as column vector
    # Returns
        a homogeneous 3 by 3 translation matrix
    """
    return translation(np.random.uniform(-1, 1, (2, 1)) * max)
...
def random_shear(max):
    ...
    """
    return shear(np.random.uniform(-max, max))
...
def random_scaling(min, max):
    ...
    """
    return scaling(np.random.uniform(-1, 1, (2, 1)) * max)
...
def transform_around(transform, center):
    ...
    """
    return np.dot(np.dot(translation_matrix(center), transform), transform_matrix(-center))
